...
def main(
    ...
    api_base = (
        api_url_input or _norm(os.getenv("DIFY_API_URL")) or "http://rmap-chatbot-demo-dify/v1"
    ).rstrip("/")
    ...
    if not dataset_id:
        return {
            "error": "DIFY_DATASET_ID not configured. Run import_dify_dsl.sh to inject from .env."
        }
    if not api_key:
        return {
            "error": "DIFY_API_KEY not configured. Run import_dify_dsl.sh to inject from .env."
        }
